...
class TestYoutubeDlAudioDownloaderOtherMethods(unittest.TestCase):
	...
	def testGetPlaylistObjectForPlaylistUrl(self):
		...
		youtubePlaylist, playlistTitle, accessError = youtubeAccess.getPlaylistObjectOrVideoTitleFortUrl(playlistUrl)

		self.assertIsNone(accessError)
		self.assertEqual(expectedPlaylistTitle, playlistTitle,)
	
	def testGetPlaylistObjectForPlaylistUrlInvalidURL(self):
		guiOutput = GuiOutputStub()
		youtubeAccess = YoutubeDlAudioDownloader(guiOutput)
	...
	def testGetPlaylistObjectForPlaylistUrl_one_time_frame_suppress(self):
		guiOutput = GuiOutputStub()
		youtubeAccess = YoutubeDlAudioDownloader(guiOutput)
	...
	def testGetPlaylistObjectForPlaylistUrl_one_time_frame_extract_suppress(self):
		guiOutput = GuiOutputStub()
		youtubeAccess = YoutubeDlAudioDownloader(guiOutput)
	...
	def testGetPlaylistObjectForPlaylistUrl_one_time_frame_suppress_extract(self):
		guiOutput = GuiOutputStub()
		youtubeAccess = YoutubeDlAudioDownloader(guiOutput)
	...
	def testGetPlaylistObjectForPlaylistUrl_multiple_time_frame_extract_suppress(self):
		guiOutput = GuiOutputStub()
		youtubeAccess = YoutubeDlAudioDownloader(guiOutput)
