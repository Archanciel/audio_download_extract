...
#		self.assertEqual(sorted(['Wear a mask Help slow the spread of Covid-19.mp4']), sorted(fileNameLst))
	...
	def testExtractAudioPortionAltLong_dir_contains_one_file_with_one_extract_timeframe(self):
		playListName = 'moviepy_explore'
		targetAudioDir = AUDIO_DIR + DIR_SEP + playListName
		...
		audioExtractor.extractAudioPortionAlt(playlistTimeFrameData)
...
#	unittest.main()
	...
	# ts = time.time()
	# tst.testExtractAudioPortionAltLong_dir_contains_one_file_with_one_extract_timeframe()
	# print(time.time() - ts)
	ts = time.time()
